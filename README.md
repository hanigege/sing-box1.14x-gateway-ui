...
内置规则更新脚本只维护运行配置真实引用的规则集：`geosite-geolocation-!cn`、`geosite-cn`、`geosite-geolocation-cn`、`geosite-icloud@cn`、`geosite-apple@cn`、`geosite-speedtest` 和 `geoip-cn`。默认先尝试 SagerNet 的 sing-box 规则集，再尝试 MetaCubeX 的 sing-box 规则集作为备用源。YouTube、Netflix、Telegram 等应用类规则如果没有被路由规则或 UI 开关引用，默认不下载；这些站点仍由 `geosite-geolocation-!cn` 覆盖走代理，只有需要为某个应用单独指定策略时，才应该新增对应 UI 开关和规则引用。规则下载越多不等于体验越好，未引用文件只会增加定时更新失败、死链和排障噪音。
